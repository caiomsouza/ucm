*------------------------------------------------------------*;
* Scorecard3: Create decision matrix;
*------------------------------------------------------------*;
data WORK.DEFAULT;
  length   DEFAULT                          $  32
           COUNT                                8
...
run;
*------------------------------------------------------------* ;
* Scorecard3: DMDBClass Macro ;
*------------------------------------------------------------* ;
%macro DMDBClass;
    DEFAULT(DESC)
%mend DMDBClass;
*------------------------------------------------------------* ;
* Scorecard3: DMDBVar Macro ;
*------------------------------------------------------------* ;
%macro DMDBVar;
...
%mend DMDBVar;
*------------------------------------------------------------*;
* Scorecard3: Create DMDB;
*------------------------------------------------------------*;
proc dmdb batch data=WORK.EM_DMREG
dmdbcat=WORK.Scorecard3_DMDB
maxlevel = 513
;
class %DMDBClass;
var %DMDBVar;
...
quit;
*------------------------------------------------------------*;
* Scorecard3: Run DMREG procedure;
*------------------------------------------------------------*;
proc dmreg data=EM_DMREG dmdbcat=WORK.Scorecard3_DMDB
validata = EMWS1.IGN_VALIDATE
testdata = EMWS1.IGN_TEST
outest = EMWS1.Scorecard3_EMESTIMATE
outterms = EMWS1.Scorecard3_OUTTERMS
outmap= EMWS1.Scorecard3_MAPDS namelen=200
;
class
DEFAULT
;
...
maxtime = 604800
;
code file="C:\Users\win\Documents\GitHub\ucm\score\trabajo3\Trabajo3_Credit_Score_SAS_EM\Workspaces\EMWS1\Scorecard3\EMPUBLISHSCORE.sas"
group=Scorecard3
;
code file="C:\Users\win\Documents\GitHub\ucm\score\trabajo3\Trabajo3_Credit_Score_SAS_EM\Workspaces\EMWS1\Scorecard3\EMFLOWSCORE.sas"
group=Scorecard3
residual
;
run;
quit;
%let note = %sysfunc(sasmsg(sashelp.credit, transformmodel_note, NOQUOTE, Scorecard3));
*------------------------------------------------------------*;
* &note;
*------------------------------------------------------------*;
%let EM_DEC_DECMETA = EMWS1.Ids_DEFAULT_DM;
%let EM_DEC_DECDATA = EMWS1.Ids_DEFAULT_DD;
%let note = %sysfunc(sasmsg(sashelp.credit, loadmacros_note, NOQUOTE, Scorecard3));
*------------------------------------------------------------*;
* &note;
*------------------------------------------------------------*;
...
%buildExportDatasets;
/* generate scorecode */
%generateScoreCode(Scorecard3);
/* apply scorecode */
%applyScoreCode;
/* generate analysis charts and plots */
%generateAnalysis;
/* generate binScoreCode */
%generateBinScoreCode(%nrbquote(C:\Users\win\Documents\GitHub\ucm\score\trabajo3\Trabajo3_Credit_Score_SAS_EM\Workspaces\EMWS1\Scorecard3\BINSCOREFILE.sas));
